...
struct ContentView: View {
    ...
    var body: some View {
        ...
        .overlay {
            if activeAPIKey.isEmpty && viewModel.items.isEmpty {
                SetupPrompt(
                    anthropicKey: $anthropicKey,
                    openAIKey: $openAIKey,
                    zaiKey: $zaiKey,
    ...
    private var canBuildAndRun: Bool {
        !activeAPIKey.isEmpty && !selectedProjectPath.isEmpty && !viewModel.isRunning
    }
    ...
    private func buildAndRun() {
        ...
        viewModel.send(prompt, apiKey: activeAPIKey, context: buildRunContext, showUserBubble: false)
    }
    ...
    private func extractBuildInfo() async {
        ...
        do {
            ...
            var json = try await LLMService().complete(
...
struct SetupPrompt: View {
    ...
    @Binding var zaiKey: String
    @Binding var zaiUseCodingEndpoint: Bool
        ...
        VStack(spacing: 20) {
            ...
            if currentProvider == .zai {
                Toggle("GLM Coding Plan API", isOn: $zaiUseCodingEndpoint)
                    .frame(maxWidth: 320)
                Text("Turn off only if you use z.ai top-up credits on the general API (not a Coding Plan subscription).")
                    .font(.caption)
                    .foregroundStyle(.secondary)
        ...
        ScrollView {
            VStack(alignment: .leading, spacing: 20) {
                ...
                VStack(alignment: .leading, spacing: 8) {
                    ...
                    SecureField("API key...", text: keyBinding)
                        .textFieldStyle(.roundedBorder)
                    Text("Stored locally. Sent only to \(currentProvider.displayName).")
                        .font(.caption)
                        .foregroundStyle(.secondary)
                }
                ...
                if currentProvider == .zai {
                    VStack(alignment: .leading, spacing: 8) {
                        Toggle("GLM Coding Plan API", isOn: $zaiUseCodingEndpoint)
                        Text(
                            "Coding Plan keys must use this endpoint. If you see “insufficient balance” (1113) with a valid subscription, ensure this is on. Turn off for pay-as-you-go credits on the general API only."
                        )
                        .font(.caption)
                        .foregroundStyle(.secondary)
                    }
