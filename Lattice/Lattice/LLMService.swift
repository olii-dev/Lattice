...
struct LLMService {
    ...
    static func latticeSystemPrompt(for context: ChatContext) -> String {
        ...
        - When the user asks for an Apple capability, use the add_capability tool. Supported capabilities: app_groups, push_notifications, storekit, keychain_sharing, background_modes, swiftdata, cloudkit_sync. The tool handles entitlements, Info.plist keys, seed files, and project build settings correctly and idempotently.
        ...
        - For capabilities outside the supported list (HealthKit, Associated Domains, Widgets, etc.), use the web_search tool to find the correct entitlement and plist keys, then explain to the user what manual steps are needed. Do not hand-write entitlements for unsupported capabilities.
